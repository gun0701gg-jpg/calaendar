...
export default function ResidentStatementModal({ onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { residents } = await parseResidentStatementFile(file);

      if (residents.length === 0) {
        setStatus("error");
        setMessage("업로드한 파일에서 수급자 데이터를 찾지 못했습니다. \"수급자명\" 열이 있는지 확인해주세요.");
        return;
      }
      ...
      setStatus("done");
      setMessage(`${residents.length}명 명세서를 생성했습니다.`);
    } catch (err) {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="modal-hint">
          국민건강보험공단 등에서 다운로드한 "청구명세 리스트"(수급자명·공단부담금·본인부담금 열이
          있는 표) 엑셀을 업로드하면, 위드온빌리지 자체 양식의 장기요양급여비용 명세서 파일로 다시
          만들어드립니다. 식사재료비·추가비용·이미용비·기타·카드결제·현금영수증·현금·이미납부한금액·비고
          항목은 원본에 없어서 빈 칸으로 만들어지니, 필요하면 만들어진 파일에서 직접 채워주세요.
        </p>
